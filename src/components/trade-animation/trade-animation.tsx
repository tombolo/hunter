...
const TradeAnimation = observer(({ className, should_show_overlay }: TTradeAnimation) => {
    ...
    // Allow Run button to work even with no saved or active bots
    const should_disable_run = false;

    const is_disabled = is_stop_button_visible ? false : shouldDisable || should_disable_run;

    // Show the tooltip only when the button is disabled
    const should_show_tooltip = false; // Always hide the tooltip since we want to allow running with default strategy
